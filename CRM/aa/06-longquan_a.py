...
class LongquanSpider(scrapy.Spider):
    ...
    sess_longquan_a = requests.session()
    ...
    def parse(self, response):
        delivery_id = 'F617B115D6F3447983E94BB781231278'
        ...
        if is_enable == 1:
            self.number += 1
            html = self.sess_longquan_a.get(url=self.start_urls[0], headers=self.headers, verify=False)
            ...
            # print('url', url)
            self.sess_longquan_a.get(url=url, headers=self.headers, verify=False)
            LoginCheck_data = {
                "intSystemID": "1",
                "strPwd": get_pwd,
            ...
            login_html = self.sess_longquan_a.post(url='{}/AjaxService.svc/LoginCheck'.format(url),
                                                   data=json.dumps(LoginCheck_data), headers=self.headers_post,
                                                   verify=False)
            login_html = self.sess_longquan_a.post(url='{}/AjaxService.svc/Repaired'.format(url),
                                                   data=json.dumps(Repaired_data), headers=self.headers_post,
                                                   verify=False)
            # print('login_html', login_html.content.decode('utf-8'))
            # print('-' * 1000)
            psot_data_html = self.sess_longquan_a.get(url='{}/Ywcx.aspx'.format(url), headers=self.headers,
                                                      verify=False)
            ...
            try:
                ...
                data_html = self.sess_longquan_a.post(url='{}/Ywcx.aspx'.format(url), data=data_data,
