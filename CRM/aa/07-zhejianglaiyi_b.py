...
class ZhejianglaiyiSpider(scrapy.Spider):
    ...
    # last = '2018-11-3'
    last = LAST
    sess_zhejianglaiyi_b = requests.session()
    ...
    def parse(self, response):
        delivery_id = 'F617B115D6F3447983E94BB781231282'
        ...
        if is_enable == 1:
            self.number += 1
            html = self.sess_zhejianglaiyi_b.get(url=self.start_urls[0], headers=self.headers, verify=False)
            image = self.sess_zhejianglaiyi_b.get(url='http://www.600216.com/lx/bmp.asp?flg=login',
            ...
            if SCRAPYD_TYPE == 1:
                ...
                if not os.path.exists(files):
                    os.makedirs(files)
                # print('path', path)
                with open(r'{}{}static{}07-zhejianglaiyi{}yzm_b.jpg'.format(path, symbol, symbol, symbol), 'wb') as f:
                    f.write(image.content)
                # 图片文件
                filename = r'{}{}static{}07-zhejianglaiyi{}yzm_b.jpg'.format(path, symbol, symbol, symbol)
            else:
                with open(r'./07-zhejianglaiyiyzm_b.jpg', 'wb') as f:
                    f.write(image.content)
                # 图片文件
                filename = r'./07-zhejianglaiyiyzm_b.jpg'
            ...
            self.sess_zhejianglaiyi_b.post(url='http://www.600216.com/lx/index.asp', data=data, headers=self.headers,
                                           verify=False)
            data_html = self.sess_zhejianglaiyi_b.get(url='http://www.600216.com/lx/sa.asp?fir=1', headers=self.headers,
                                                      verify=False)
            try:
                re.findall(r'销售明细查询', data_html.content.decode('gb2312'))[0]
                # print('data_html.content.decode', data_html.content.decode('gb2312'))
                for i in range(1, 1000):
                    data_html = self.sess_zhejianglaiyi_b.get(
                ...
                for i in range(1, 1000):
                    data_html_cai = self.sess_zhejianglaiyi_b.get(
                ...
                sql_data_crm_record = sql_crm_record.format(company_id, delivery_id, enterprise_name,
                                                            get_account, '07-zhejianglaiyi_b',
                ...
                print('zhejianglaiyi_b-登入失败:%s' % e)
                if self.number < 4:
                    self.parse('aa')
                else:
                    create_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
