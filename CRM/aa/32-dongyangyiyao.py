...
class LongquanSpider(scrapy.Spider):
    ...
    # last = '2018-11-01'
    last = LAST
    sess_dongyangyiyao = requests.session()
    ...
    def parse(self, response):
        delivery_id = 'F617B115D6F3447983E94BB781231281'
        ...
        if is_enable == 1:
            self.number += 1
            html = self.sess_dongyangyiyao.get(url=self.start_urls[0], headers=self.headers, verify=False)
            post_data = {
            ...
            login_html = self.sess_dongyangyiyao.post(url='http://www.dyyyyc.com/LoginCheck.asp',
                                                      data=post_data, headers=self.headers, verify=False)
            # print('login_html', login_html.content.decode('gb2312'))
            # print('*'*1000)
            # print('post_data', post_data)
            psot_data_html = self.sess_dongyangyiyao.get(url='http://www.dyyyyc.com/Splsk_Quevy.asp?A=11',
            ...
            try:
                ...
                for i in range(len(data_list)):
                    ...
                    data_html = self.sess_dongyangyiyao.post(
                    ...
                    for j in range(1, page_num):
                        data_html = self.sess_dongyangyiyao.get(
