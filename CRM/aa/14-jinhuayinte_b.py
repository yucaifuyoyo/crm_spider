...
class JinhuayinteSpider(scrapy.Spider):
    ...
    last = LAST
    # last = '2018-11-01'
    sess_jinhuayinte_b = requests.session()
    ...
    def parse(self, response):
        delivery_id = 'F617B115D6F3447983E94BB781231277'
        ...
        if is_enable == 1:
            self.number += 1
            html = self.sess_jinhuayinte_b.get(url=self.start_urls[0], headers=self.headers, verify=False)
            image = self.sess_jinhuayinte_b.get(
            ...
            if SCRAPYD_TYPE == 1:
                ...
                if not os.path.exists(files):
                    os.makedirs(files)
                with open(r'{}{}static{}14-jinhuayinte{}yzm_b.jpg'.format(path, symbol, symbol, symbol), 'wb') as f:
                    f.write(image.content)
                # 图片文件
                filename = r'{}{}static{}14-jinhuayinte{}yzm_b.jpg'.format(path, symbol, symbol, symbol)
            else:
                with open(r'./14-jinhuayinteyzm_b.jpg', 'wb') as f:
                    f.write(image.content)
                # 图片文件
                filename = r'./14-jinhuayinteyzm_b.jpg'
            ...
            try:
                ...
                data_html_cai = self.sess_jinhuayinte_b.get(
