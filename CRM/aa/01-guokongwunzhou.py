...
class GuokaonSpider(scrapy.Spider):
    ...
    sess_guokongwunzhou = requests.Session()
    headers = HEADERS
    ...
    def parse(self, response):
        # delivery_id = 'F617B115D6F3447983E94BB781231266'
        delivery_id = 'DDA1001001'
        ...
        if is_enable == 1:
            self.number += 1
            html = self.sess_guokongwunzhou.get(url=self.start_urls[0], headers=self.headers, verify=False)
            ...
            image = self.sess_guokongwunzhou.get(url='http://data.wzshengwu.com/CheckCode.aspx', headers=self.headers,
                                                 verify=False)
            if SCRAPYD_TYPE == 1:
                ...
                if 'indow' in platform.system():
                    ...
                    symbol = r'/'
                files = r'{}{}static{}01-guokao'.format(path, symbol, symbol)
                if not os.path.exists(files):
                    os.makedirs(files)
                with open(r'{}{}static{}01-guokao{}yzm.jpg'.format(path, symbol, symbol, symbol), 'wb') as f:
                    f.write(image.content)
                # 图片文件
                filename = r'{}{}static{}01-guokao{}yzm.jpg'.format(path, symbol, symbol, symbol)
            else:
                with open(r'./01-guokaoyzm.jpg', 'wb') as f:
                    f.write(image.content)
                # 图片文件
                filename = r'./01-guokaoyzm.jpg'
            codetype = 1004
            # 超时时间，秒
            timeout = 60
            ydm = YDMHttp()
            ...
            repost = self.sess_guokongwunzhou.post(url=uel, data=data, headers=self.headers, verify=False)

            data_get = self.sess_guokongwunzhou.get(url='http://data.wzshengwu.com/SaleQuery.aspx',
            ...
            try:
                ...
                datas = {
                    ...
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl04$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl05$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl06$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl07$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl08$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl09$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl10$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl11$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl12$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl13$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl14$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl15$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl16$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl17$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl18$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl19$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl20$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl21$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl22$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl23$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl24$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl25$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl26$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl27$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl28$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl29$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl30$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl31$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl32$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl33$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl34$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl35$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl36$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl37$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl38$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl39$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl40$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl41$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl42$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl43$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl44$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl45$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl46$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl47$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl48$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl49$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl50$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl51$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl52$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl53$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl54$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl55$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl56$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl57$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl58$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl59$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl60$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl61$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl62$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl63$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl64$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl65$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl66$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl67$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl68$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl69$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl70$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl71$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl72$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl73$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl74$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl75$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl76$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl77$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl78$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl79$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl80$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl81$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl82$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl83$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl84$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl85$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl86$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl87$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl88$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl89$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl90$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl91$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl92$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl93$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl94$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl95$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl96$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl97$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl98$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl99$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl100$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl101$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl102$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl103$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl104$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl105$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl106$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl107$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl108$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl109$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl110$CheckName": "on",
                    # "ctl00$ContentPlaceHolder1$GridView2$ctl111$CheckName": "on",
                }
                CheckName_value = []
                ...
                # print('datas', datas)
                data_html = self.sess_guokongwunzhou.post(url='http://data.wzshengwu.com/SaleQuery.aspx', data=datas,
                ...
                for i in range(data_len):
                    ...
                    if drug_name != 1:
                        ...
                        table_name = 'order_metadata_guokongwunzhou'
                        ...
                        # print('hospital_hashs', hospital_hashs)
                        # print('hospital_hash', hospital_hash)
                        # print('stream_hashs', stream_hashs)
                        # print('stream_hash', stream_hash)
                        month = int(str(self.fist).replace('-', '')[0: 6])

                        sql_crm = "insert into order_metadata_guokongwunzhou(company_id, delivery_id, delivery_name, data_version, data_type, bill_type, drug_name, drug_specification, supplier_name, drug_unit, drug_number, drug_batch, valid_till, hospital_name, hospital_address, sell_time, create_time, update_time, drug_price, drug_hash, hospital_hash, stream_hash, month) values('{}', '{}', '{}', '{}', {}, '{}', '{}', '{}', '{}', '{}', {}, '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', {})"
                        ...
                        self.cursor.execute('select max(id) from order_metadata_guokongwunzhou')
                ...
                # -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                data_get_jin = self.sess_guokongwunzhou.get(url='http://data.wzshengwu.com/PurList.aspx?type=00',
                                                            headers=self.headers,
                                                            verify=False)
                ...
                data_html_jin = self.sess_guokongwunzhou.post(url='http://data.wzshengwu.com/PurList.aspx?type=00',
                                                              data=datas_jin,
                                                              headers=self.headers,
                                                              verify=False)
                ...
                # ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                data_get_tui = self.sess_guokongwunzhou.get(url='http://data.wzshengwu.com/PurList.aspx?type=00',
                                                            headers=self.headers,
                                                            verify=False)
                ...
                data_html_tui = self.sess_guokongwunzhou.post(url='http://data.wzshengwu.com/PurList.aspx?type=10',
                                                              data=datas_tui,
                                                              headers=self.headers,
                                                              verify=False)
                ...
                if MONTHS == 0:
                    self.cursor.execute(
                        "SELECT count(*) from order_metadata_guokongwunzhou WHERE sell_time='{}' and delivery_name='{}'".format(
                            self.yesterday, enterprise_name))
                else:
                    month = int(str(self.fist).replace('-', '')[0: 6])
                    self.cursor.execute(
                        "SELECT count(*) from order_metadata_guokongwunzhou WHERE month='{}' and delivery_name='{}'".format(
