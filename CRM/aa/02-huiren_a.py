...
class HuirenSpider(scrapy.Spider):
    ...
    fist = FIST
    last = LAST
    sess_huiren_a = requests.session()
    ...
    def parse(self, response):
        delivery_id = 'F617B115D6F3447983E94BB781231268'
        ...
        if is_enable == 1:
            ...
            res1 = self.sess_huiren_a.get(login_url, headers=self.headers)
            ...
            res2 = self.sess_huiren_a.get(code_url, headers=headers)
            # print(res2.text)
            if SCRAPYD_TYPE == 1:
                ...
                files = r'{}{}static{}02-huiren'.format(path, symbol, symbol)
                if not os.path.exists(files):
                    os.makedirs(files)
                with open(r'{}{}static{}02-huiren{}yzm_a.jpg'.format(path, symbol, symbol, symbol), 'wb') as f:
                    f.write(res2.content)
                filename = r'{}{}static{}02-huiren{}yzm_a.jpg'.format(path, symbol, symbol, symbol)
            else:
                with open(r'./02-huirenyzm_a.jpg', 'wb') as f:
                    f.write(res2.content)
                filename = r'./02-huirenyzm_a.jpg'
            codetype = 4000
            # 超时时间，秒
            timeout = 60
            ydm = YDMHttp()
            ...
            res3 = self.sess_huiren_a.post(post_url, data=post_data, headers=headers)
            res4 = self.sess_huiren_a.get("http://www.zjhuiren.com/manager.asp?imark=1&ID=1888", headers=headers)
            ...
            data_resp = self.sess_huiren_a.post(url='http://www.zjhuiren.com/gjmx.asp?action=goselect&ID=1888',
                                                data=data,
            ...
            try:
                ...
                for i in range(1, page):
                    data_resp = self.sess_huiren_a.get(
                        ...
                        # data_resp = self.sess_huiren_a.get(url='http://www.zjhuiren.com/gjmx.asp?Page={}&ID=1888&bgtime={}&ovtime={}&spbm=&tym=&dwmch=&px=rq&oldshow=purchase'.format(i, '2018-10-01', '2018-10-15'),
                        headers=self.headers,
                        verify=False)
                    # print('*' * 1000)
                ...
                data_resp = self.sess_huiren_a.post(url='http://www.zjhuiren.com/xsmx.asp?action=goselect&ID=1888',
                                                    data=data,
                                                    headers=self.headers,
                                                    verify=False)
                try:
                ...
                for i in range(1, page):
                    ...
                    url = 'http://www.zjhuiren.com/xsmx.asp?Page={}&ID=1888&bgtime={}&ovtime={}&spbm=&tym=&dwmch=&px=rq&oldshow=sale'.format(
                        i, self.fist, self.last)
                    # print(url)
                    data_resp = self.sess_huiren_a.get(url=url, headers=self.headers, verify=False)
                    # print('*' * 1000)
                    data_resps = etree.HTML(data_resp.content.decode('utf-8'))
                    data_len = data_resps.xpath('/html/body/table[2]/tr')
                    # print(data_len)
