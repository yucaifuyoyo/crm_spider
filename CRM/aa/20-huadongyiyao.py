...
class EastpharmSpider(scrapy.Spider):
    ...
    sess_huadongyiyao = requests.Session()
    headers = HEADERS
    ...
    def parse(self, response):
        delivery_id = 'F617B115D6F3447983E94BB781231267'
        ...
        if is_enable == 1:
            self.number += 1
            html = self.sess_huadongyiyao.get(url=self.start_urls[0], headers=self.headers, verify=False)
            ...
            res = self.sess_huadongyiyao.get("http://lx.eastpharm.com/Confirmation.aspx", headers=self.headers,
                                             verify=False)
            ...
            self.sess_huadongyiyao.post(url='http://lx.eastpharm.com/web_login.aspx?l=1&ReturnUrl=%2fdefault.aspx',
                                        headers=self.headers, data=post_data)
            data_get = self.sess_huadongyiyao.get(url='http://lx.eastpharm.com/Flow_Query.aspx', headers=self.headers,
                                                  verify=False)
            ...
            try:
                ...
                post_resp = self.sess_huadongyiyao.post(url='http://lx.eastpharm.com/Flow_Query.aspx',
                ...
                for j in range(1, page_num):
                    ...
                    for i in range(data_len):
                        ...
                        if drug_name != 1:
                            ...
                            table_name = 'order_metadata_huadongyiyao'
                            ...
                            self.cursor.execute('select max(id) from order_metadata_huadongyiyao')
                ...
                if MONTHS == 0:
                    self.cursor.execute(
                        "SELECT count(*) from order_metadata_huadongyiyao WHERE sell_time='{}' and delivery_name='{}'".format(
                            self.yesterday, enterprise_name))
                else:
                    month = int(str(self.fist).replace('-', '')[0: 6])
                    self.cursor.execute(
                        "SELECT count(*) from order_metadata_huadongyiyao WHERE month='{}' and delivery_name='{}'".format(
