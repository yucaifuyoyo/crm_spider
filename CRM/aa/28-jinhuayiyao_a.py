...
class NingboyinteSpider(scrapy.Spider):
    ...
    last = LAST
    # last = '2018-11-2'
    sess_jinhuayiyao_a = requests.session()
    ...
    def parse(self, response):
        delivery_id = 'F617B115D6F3447983E94BB781231276'
        ...
        if is_enable == 1:
            self.number += 1
            html = self.sess_jinhuayiyao_a.get(url=self.start_urls[0], headers=self.headers, verify=False)
            ...
            post_html = self.sess_jinhuayiyao_a.post(url='http://www.jhyynet.com:8008/default.aspx', data=data,
                                                     headers=self.headers,
                                                     verify=False)
            # print('*' * 1000)
            # print('post_html', post_html.content.decode('gb2312', 'ignore'))
            self.sess_jinhuayiyao_a.get(url='http://www.jhyynet.com:8008/Search/ProductFlow.aspx',
                                        headers=self.headers, verify=False)
            data_get = self.sess_jinhuayiyao_a.get(url='http://www.jhyynet.com:8008/Search/ProductFlow.aspx',
                                                   headers=self.headers, verify=False)
            try:
                ...
                data_html = self.sess_jinhuayiyao_a.post(url='http://www.jhyynet.com:8008/Search/ProductFlow.aspx',
                                                         data=datas,
                                                         headers=self.headers,
                                                         verify=False)
                ...
                try:
                    # print(data_html.xpath('//*[@id="DgdFlow"]/tr[18]/td/a/text()'))
                ...
                # ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                data_get = self.sess_jinhuayiyao_a.get(url='http://www.jhyynet.com:8008/Search/PurSearch.aspx',
                                                       headers=self.headers, verify=False)
                data_get = etree.HTML(data_get.content.decode('gb2312'))
                __VIEWSTATE = data_get.xpath('//*[@id="__VIEWSTATE"]/@value')[0]
                __EVENTVALIDATION = data_get.xpath('//*[@id="__EVENTVALIDATION"]/@value')[0]
                ...
                data_html = self.sess_jinhuayiyao_a.post(url='http://www.jhyynet.com:8008/Search/PurSearch.aspx',
                                                         data=datas,
                                                         headers=self.headers,
                                                         verify=False)
