...
class EastpharmSpider(scrapy.Spider):
    ...
    sess_huadongshangwu = requests.Session()
    headers = HEADERS
    ...
    def parse(self, response):
        # delivery_id = 'F617B115D6F3447983E94BB781231241'
        # F617B115D6F3447983E94BB781231267
        # delivery_id = 'DDA100100C'
        delivery_id = 'DDA1001005'
        ...
        if is_enable == 1:
            self.number += 1
            html = self.sess_huadongshangwu.get(url=self.start_urls[0], headers=self.headers, verify=False)
            ...
            res = self.sess_huadongshangwu.get("http://lx.eastpharm.com/Confirmation.aspx", headers=self.headers,
                                               verify=False)
            ...
            self.sess_huadongshangwu.post(url='http://lx.eastpharm.com/web_login.aspx?l=1&ReturnUrl=%2fdefault.aspx',
                                          headers=self.headers, data=post_data)
            data_get = self.sess_huadongshangwu.get(url='http://lx.eastpharm.com/Flow_Query.aspx', headers=self.headers,
                                                    verify=False)
            ...
            try:
                ...
                post_resp = self.sess_huadongshangwu.post(url='http://lx.eastpharm.com/Flow_Query.aspx',
                ...
                for j in range(1, page_num):
                    ...
                    for i in range(data_len):
                        ...
                        if drug_name != 1:
                            ...
                            table_name = 'order_metadata_huadongshangwu'
                            ...
                            sql_crm = "insert into order_metadata_huadongshangwu(company_id, delivery_id, delivery_name, data_version, data_type, bill_type, drug_name, drug_specification, supplier_name, drug_unit, drug_number, drug_batch, valid_till, area, customer_type, hospital_name, hospital_address, sell_time, create_time, update_time, drug_price, drug_hash, hospital_hash, stream_hash, month) values('{}', '{}', '{}', '{}', {}, '{}', '{}', '{}', '{}', '{}', {}, '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', {})"
                            ...
                            self.cursor.execute('select max(id) from order_metadata_huadongshangwu')
                ...
                if MONTHS == 0:
                    self.cursor.execute(
                        "SELECT count(*) from order_metadata_huadongshangwu WHERE sell_time='{}' and delivery_name='{}'".format(
                            self.yesterday, enterprise_name))
                else:
                    month = int(str(self.fist).replace('-', '')[0: 6])
                    self.cursor.execute(
                        "SELECT count(*) from order_metadata_huadongshangwu WHERE month='{}' and delivery_name='{}'".format(
