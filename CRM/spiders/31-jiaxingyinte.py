...
class JinhuayinteSpider(scrapy.Spider):
    ...
    # last = '2018-10-31'
    last = LAST
    sess_jiaxingyinte = requests.session()
    ...
    def parse(self, response):
        ...
        if is_enable == 1:
            self.number += 1
            html = self.sess_jiaxingyinte.get(url=self.start_urls[0], headers=self.headers, verify=False)
            ...
            if SCRAPYD_TYPE == 1:
                ...
                # print('path', path)
                files = r'{}{}static{}31-jiaxingyinte'.format(path, symbol, symbol)
                if not os.path.exists(files):
                    os.makedirs(files)
                with open(r'{}{}static{}31-jiaxingyinte{}yzm.jpg'.format(path, symbol, symbol, symbol), 'wb') as f:
                    f.write(image.content)
                # 图片文件
                filename = r'{}{}static{}31-jiaxingyinte{}yzm.jpg'.format(path, symbol, symbol, symbol)
            else:
                with open(r'./31-jiaxingyinteyzm.jpg', 'wb') as f:
                    f.write(image.content)
                # 图片文件
                filename = r'./31-jiaxingyinteyzm.jpg'
            ...
            data_html = self.sess_jiaxingyinte.get(url='http://www.drugoogle.com/member/index.jspx?catlog=4',
                                                   headers=self.headers,
                                                   verify=False)
            try:
                ...
                data_html = self.sess_jiaxingyinte.get(
                    url='http://www.drugoogle.com/member/agentman/medicineGoto/medicinegototab4.jspx?entryId=16&medicineId=0&company_name=&timeType=1&startTime={}%2000:00:00&endTime={}%2023:59:59&buyerType=0'.format(
                        self.fist, self.last),
                ...
                for i in range(data_len):
                    ...
                    if drug_name != 1:
                        ...
                        self.cursor.execute('select max(id) from order_metadata_jiaxingyinte')
                ...
                data_html_cai = self.sess_jiaxingyinte.get(
                    url='http://www.drugoogle.com/member/agentman/medicineGoto/medicinegototab3.jspx?entryId=16&medicineId=0&timeType=1&startTime={}%2000:00:00&endTime={}%2023:59:59'.format(
                        self.fist, self.last),
                ...
                if MONTHS == 0:
                    self.cursor.execute(
                        "SELECT count(*) from order_metadata_jiaxingyinte WHERE sell_time='{}' and delivery_name='{}'".format(
                            self.yesterday, enterprise_name))
                else:
                    month = int(str(self.fist).replace('-', '')[0: 6])
                    self.cursor.execute(
                        "SELECT count(*) from order_metadata_jiaxingyinte WHERE month='{}' and delivery_name='{}'".format(
