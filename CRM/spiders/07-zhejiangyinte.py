...
class YingtehaishiSpider(scrapy.Spider):
    name = 'zhejiangyinte'
    allowed_domains = ['.com']
    ...
    fist = FIST
    last = LAST
    sess_zhejiangyinte = requests.session()
    ...
    def parse(self, response):
        # delivery_id = 'F617B115D6F3447983E94BB781231238'
        delivery_id = 'DDA1001008'
        ...
        if is_enable == 1:
            self.number += 1
            html = self.sess_zhejiangyinte.get(url=self.start_urls[0], headers=self.headers, verify=False)
            image = self.sess_zhejiangyinte.get(
            ...
            if SCRAPYD_TYPE == 1:
                ...
                # print('path', path)
                files = r'{}{}static{}07-zhejiangyinte'.format(path, symbol, symbol)
                if not os.path.exists(files):
                    os.makedirs(files)
                with open(r'{}{}static{}07-zhejiangyinte{}yzm.jpg'.format(path, symbol, symbol, symbol), 'wb') as f:
                    f.write(image.content)
                # 图片文件
                filename = r'{}{}static{}07-zhejiangyinte{}yzm.jpg'.format(path, symbol, symbol, symbol)
            else:
                with open(r'./07-zhejiangyinteyzm.jpg', 'wb') as f:
                    f.write(image.content)
                # 图片文件
                filename = r'./07-zhejiangyinteyzm.jpg'
            ...
            data = {"username": get_account,
                    ...
                    "verifyCode": yzm}
            # print('data', data)
            self.sess_zhejiangyinte.post(
                url="http://www.drugoogle.com/index/registerloginjson.jspx?%s" % (int(time.time() * 1000)),
                data=data, headers=self.headers, verify=False)
            data_html = self.sess_zhejiangyinte.get(url='http://www.drugoogle.com/member/index.jspx?catlog=4',
                                                    headers=self.headers,
                                                    verify=False)
            try:
                # print("data_html.content.decode('utf-8')", data_html.content.decode('utf-8'))
                re.findall(r'药品流向查询', data_html.content.decode('utf-8'))[0]
                data_html = self.sess_zhejiangyinte.get(
                    url='http://www.drugoogle.com/member/agentman/medicineGoto/medicinegototab4.jspx?entryId=3&medicineId=0&company_name=&timeType=2&startTime={}%2000:00:00&endTime={}%2023:59:59&buyerType=0'.format(
                        self.fist, self.last),
                    # url='http://www.drugoogle.com/member/agentman/medicineGoto/medicinegototab4.jspx?entryId=3&medicineId=0&company_name=&timeType=2&startTime=2018-05-01%2000:00:00&endTime=2018-10-19%2023:59:59&buyerType=0',
                    ...
                    if drug_name != 1:
                        ...
                        try:
                            ...
                            hospital_name = data_html.xpath(
                                '/html/body/table/tr/td/table[1]/tr[%s]/td[13]/text()' % (i + 2))[0].strip()
                        except:
                            hospital_name = ''
                        ...
                        try:
                            ...
                            hospital_address = data_html.xpath(
                                '/html/body/table/tr/td/table[1]/tr[%s]/td[15]/text()' % (i + 2))[0].strip()
                        except:
                            hospital_address = ''
                        ...
                        try:
                            ...
                            drug_price = data_html.xpath(
                                '/html/body/table/tr/td/table[1]/tr[%s]/td[11]/text()' % (i + 2))[0].split(' ')[
                                0].strip()
                        except:
                            drug_price = ''
                        ...
                        table_name = 'order_metadata_zhejiangyinte'
                        ...
                        sql_crm = "insert into order_metadata_zhejiangyinte(company_id, delivery_id, delivery_name, data_version, data_type, bill_type, drug_name, trade_id, sales_ticket_id, drug_specification, supplier_name, drug_unit, drug_number, drug_batch, valid_till, hospital_name, hospital_address, sell_time, warehouse_time, create_time, update_time, drug_hash, hospital_hash, stream_hash, drug_price, month) values('{}', '{}', '{}', '{}', {}, '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', {})"
                        ...
                        self.cursor.execute('select max(id) from order_metadata_zhejiangyinte')
                ...
                data_html_cai = self.sess_zhejiangyinte.get(
                    url='http://www.drugoogle.com/member/agentman/medicineGoto/medicinegototab3.jspx?entryId=3&medicineId=0&timeType=1&startTime={}%2000:00:00&endTime={}%2023:59:59'.format(
                        self.fist, self.last),
                    # url='http://www.drugoogle.com/member/agentman/medicineGoto/medicinegototab3.jspx?entryId=3&medicineId=0&timeType=1&startTime=2018-05-01%2000:00:00&endTime=2018-10-19%2023:59:59',
                    headers=self.headers, verify=False)
                ...
                if MONTHS == 0:
                    self.cursor.execute(
                        "SELECT count(*) from order_metadata_zhejiangyinte WHERE sell_time='{}' and delivery_name='{}'".format(
                            self.yesterday, enterprise_name))
                else:
                    month = int(str(self.fist).replace('-', '')[0: 6])
                    self.cursor.execute(
                        "SELECT count(*) from order_metadata_zhejiangyinte WHERE month='{}' and delivery_name='{}'".format(
