...
class JinhuayinteSpider(scrapy.Spider):
    ...
    fist = FIST
    last = LAST
    sess_jinhuayinte = requests.session()
    ...
    def parse(self, response):
        # delivery_id = 'F617B115D6F3447983E94BB781231247'
        delivery_id = 'DDA1001017'
        ...
        if is_enable == 1:
            self.number += 1
            html = self.sess_jinhuayinte.get(url=self.start_urls[0], headers=self.headers, verify=False)
            image = self.sess_jinhuayinte.get(
            ...
            if SCRAPYD_TYPE == 1:
                ...
                # print('path', path)
                files = r'{}{}static{}24-jinhuayinte'.format(path, symbol, symbol)
                if not os.path.exists(files):
                    os.makedirs(files)
                with open(r'{}{}static{}24-jinhuayinte{}yzm.jpg'.format(path, symbol, symbol, symbol), 'wb') as f:
                    f.write(image.content)
                # 图片文件
                filename = r'{}{}static{}24-jinhuayinte{}yzm.jpg'.format(path, symbol, symbol, symbol)
            else:
                with open(r'./24-jinhuayinteyzm.jpg', 'wb') as f:
                    f.write(image.content)
                # 图片文件
                filename = r'./24-jinhuayinteyzm.jpg'
            ...
            try:
                ...
                data_html_cai = self.sess_jinhuayinte.get(
