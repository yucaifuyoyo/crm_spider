...
class GuokonglishuiSpider(scrapy.Spider):
    ...
    sess_guokongzhejiang = requests.session()
    ...
    def parse(self, response):
        # delivery_id = 'F617B115D6F3447983E94BB781231284'
        delivery_id = 'DDA1001002'
        ...
        if is_enable == 1:
            self.number += 1
            html = self.sess_guokongzhejiang.get(url=self.start_urls[0], headers=self.headers, verify=False)
            ...
            post_html = self.sess_guokongzhejiang.post(url='http://www.gkzj.com:81/modal/login.php', data=data,
            ...
            data_html = self.sess_guokongzhejiang.post(url='http://www.gkzj.com:81/modal/order.json.php',
                                                       data=data_data,
                                                       headers=self.headers,
                                                       verify=False)
            # print('*' * 1000)
            ...
            try:
                ...
                for i in range(1, data_num):
                    ...
                    for data_resps in json.loads(data_html.content.decode('utf-8'))['rows']:
                        ...
                        if drug_name != 1:
                            ...
                            table_name = 'order_metadata_guokongzhejiang'
                            ...
                            sql_crm = "insert into order_metadata_guokongzhejiang(company_id, delivery_id, delivery_name, data_version, data_type, bill_type, drug_name, drug_specification, supplier_name, drug_unit, drug_number, hospital_type, drug_batch, valid_till, hospital_name, hospital_address, sell_time, create_time, update_time, drug_price, channel_name, drug_hash, hospital_hash, stream_hash, month) values('{}', '{}', '{}', '{}', {}, '{}', '{}', '{}', '{}', '{}', {}, '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', {})"
                            ...
                            self.cursor.execute('select max(id) from order_metadata_guokongzhejiang')
                ...
                if MONTHS == 0:
                    self.cursor.execute(
                        "SELECT count(*) from order_metadata_guokongzhejiang WHERE sell_time='{}' and delivery_name='{}'".format(
                            self.yesterday, enterprise_name))
                else:
                    month = int(str(self.fist).replace('-', '')[0: 6])
                    self.cursor.execute(
                        "SELECT count(*) from order_metadata_guokongzhejiang WHERE month='{}' and delivery_name='{}'".format(
                ...
                sql_data_crm_record = sql_crm_record.format(company_id, delivery_id, enterprise_name,
                                                            get_account, '02-guokongzhejiang',
                ...
                print('guokongzhejiang-登入失败:%s' % e)
