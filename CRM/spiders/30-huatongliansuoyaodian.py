...
class LongquanSpider(scrapy.Spider):
    name = 'huatongliansuoyaodian'
    # allowed_domains = ['.']
    start_urls = ['http://60.12.99.78:9663/ht/login.aspx?I=0']
    headers = HEADERS
    time_stamp = str(int(time.time() * 1000))
    yesterday = YESTERDAY
    fist = FIST
    last = LAST
    sess_huatongliansuoyaodian = requests.session()
    ...
    def parse(self, response):
        # delivery_id = 'F617B115D6F3447983E94BB781231274'
        delivery_id = 'DDA100101N'
        ...
        if is_enable == 1:
            self.number += 1
            html = self.sess_huatongliansuoyaodian.get(url=self.start_urls[0], headers=self.headers, verify=False)
            ...
            image = self.sess_huatongliansuoyaodian.get(url='http://60.12.99.78:9663/ht/Confirmation.aspx',
            ...
            login_html = self.sess_huatongliansuoyaodian.post(url='http://60.12.99.78:9663/ht/login.aspx?I=0',
                                                              data=Login_data, headers=self.headers, verify=False)
            # print('login_html', login_html.content.decode('utf-8'))
            # print('Login_data', Login_data)
            psot_data_html = self.sess_huatongliansuoyaodian.get(
                url='http://60.12.99.78:9663/ht/QueryModel.aspx?Qid=76067&Qstr=59257',
            ...
            try:
                ...
                data_data = {
                    ...
                    "ctl00$ContentPlaceHolder3$ZBSQL": "QN_SEND_LIST_FOR_OUTER 117,'59257,,%s,%s',',10,1'" % (
                        self.fist, self.last),
                    "ctl00$ContentPlaceHolder3$ZBALLCOLS": "sortid|GoodsName|DataKey|batchNo|BusiType|BusiDate|Amount|Custom|ValidDate",
                    "ctl00$ContentPlaceHolder3$ZBPAGEKEY": "59257",
                    "ctl00$ContentPlaceHolder3$SHOWSQL76067": "QN_SEND_LIST_FOR_OUTER 117,'59257,,%s,%s',',10,1'" % (
                        self.fist, self.last),
                    "ctl00$ContentPlaceHolder3$ZBInfoPanel1_PARA": "59257",
                    "ctl00$ContentPlaceHolder3$ZBEdit1": "",
                    # "ctl00$ContentPlaceHolder3$ZBDate1": "2017-11-05",
                    ...
                    "ctl00$ContentPlaceHolder3$ctl07": "1",
                    "ctl00$ContentPlaceHolder3$ctl08": "3",
                    "__VIEWSTATEGENERATOR": __VIEWSTATEGENERATOR,
                    "__ASYNCPOST": "true",
                    "ctl00$ContentPlaceHolder3$UDQRefreshButton": "查询",
                }
                data_html = self.sess_huatongliansuoyaodian.post(
                    url='http://60.12.99.78:9663/ht/QueryModel.aspx?Qid=76067&Qstr=59257',
                ...
                data_len = int(len(data_html.xpath('//*[@id="ContentPlaceHolder3_GridBlock76067"]/tr'))) - 2
                # print(data_len)
                md5 = hashlib.md5()
                for i in range(data_len):
                    ...
                    try:
                        drug_name = data_html.xpath(
                            '//*[@id="ContentPlaceHolder3_GridBlock76067"]/tr[%s]/td[2]/text()' % (i + 2))[0].strip()
                        if not drug_name:
                            drug_name = 1
                    except:
                        drug_name = 1
                    ...
                    if drug_name != 1:
                        ...
                        try:
                            types = data_html.xpath(
                                '//*[@id="ContentPlaceHolder3_GridBlock76067"]/tr[%s]/td[5]/text()' % (i + 2))[
                                0].strip()
                            # 单据类型:1进货,2退货,3销售,4销售退货
                            if types == '配送':
                                bill_type = 3
                        ...
                        try:
                            ...
                            drug_number = round(float(data_html.xpath(
                                '//*[@id="ContentPlaceHolder3_GridBlock76067"]/tr[%s]/td[7]/text()' % (i + 2))[
                                                          0].strip()))
                        except:
                            drug_number = 0
                        ...
                        try:
                            ...
                            drug_batch = data_html.xpath(
                                '//*[@id="ContentPlaceHolder3_GridBlock76067"]/tr[%s]/td[4]/text()' % (i + 2))[
                                0].strip()
                        except:
                            drug_batch = ''
                        ...
                        try:
                            ...
                            valid_till = data_html.xpath(
                                '//*[@id="ContentPlaceHolder3_GridBlock76067"]/tr[%s]/td[9]/text()' % (i + 2))[
                                0].strip()
                        except:
                        ...
                        try:
                            ...
                            hospital_name = data_html.xpath(
                                '//*[@id="ContentPlaceHolder3_GridBlock76067"]/tr[%s]/td[8]/text()' % (i + 2))[
                                0].strip()
                        except:
                            hospital_name = ''
                        ...
                        try:
                            ...
                            sell_time = data_html.xpath(
                                '//*[@id="ContentPlaceHolder3_GridBlock76067"]/tr[%s]/td[6]/text()' % (i + 2))[
                                0].strip().split(' ')[0]
                        except:
                        ...
                        table_name = 'order_metadata_huatongliansuoyaodian'
                        ...
                        sql_crm = "insert into order_metadata_huatongliansuoyaodian(company_id, delivery_id, delivery_name, data_version, data_type, bill_type, drug_name, drug_specification, supplier_name, drug_unit, drug_number, drug_batch, valid_till, hospital_name, hospital_address, sell_time, create_time, update_time, drug_hash, hospital_hash, stream_hash, month) values('{}', '{}', '{}', '{}', {}, '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', {})"
                        ...
                        self.cursor.execute('select max(id) from order_metadata_huatongliansuoyaodian')
                ...
                psot_data_html = self.sess_huatongliansuoyaodian.get(
                    url='http://60.12.99.78:9663/ht/QueryModel.aspx?Qid=76063&Qstr=59257',
                    headers=self.headers, verify=False)
                # print('psot_data_html', psot_data_html.content.decode('utf-8'))
                # print('*' * 1000)
                psot_data_html = etree.HTML(psot_data_html.content.decode('utf-8'))
                ...
                data_data = {
                    ...
                    "ctl00$ContentPlaceHolder3$ZBDEFAULTVALUE": "",
                    "ctl00$ContentPlaceHolder3$ZBDATAKEYS": "",
                    "ctl00$ContentPlaceHolder3$ZBSQL": "QN_IN_LIST_FOR_OUTER 117,'59257,,%s,%s',',100,1'" % (
                        self.fist, self.last),
                    "ctl00$ContentPlaceHolder3$ZBALLCOLS": "sortid|GoodsName|DataKey|batchNo|BusiType|BusiDate|Amount|ValidDate|Provider|old_no",
                    "ctl00$ContentPlaceHolder3$ZBPAGEKEY": "59257",
                    "ctl00$ContentPlaceHolder3$SHOWSQL76063": "QN_IN_LIST_FOR_OUTER 117,'59257,,%s,%s',',100,1'" % (
                        self.fist, self.last),
                    "ctl00$ContentPlaceHolder3$ZBInfoPanel1_PARA": "59257",
                    "ctl00$ContentPlaceHolder3$ZBEdit2": "",
                    # "ctl00$ContentPlaceHolder3$ZBDate1": "2014-12-29",
                    "ctl00$ContentPlaceHolder3$ZBDate1": self.fist,
                    # "ctl00$ContentPlaceHolder3$ZBDate2": "2018-10-31",
                    "ctl00$ContentPlaceHolder3$ZBDate2": self.last,
                    "ctl00$ContentPlaceHolder3$WA1_ZBGridPageSize": "100",
                    ...
                    "ctl00$ContentPlaceHolder3$WA2_ZBToPage": "1",
                    "ctl00$ContentPlaceHolder3$ctl07": "1",
                    "ctl00$ContentPlaceHolder3$ctl08": "1",
                    "__EVENTTARGET": "",
                    "__EVENTARGUMENT": "",
                    "__VIEWSTATE": __VIEWSTATE,
                    "__VIEWSTATEGENERATOR": __VIEWSTATEGENERATOR,
                ...
                data_html = etree.HTML(data_html.content.decode('utf-8'))
                # //*[@id="ContentPlaceHolder3_GridBlock76067"]/tbody/tr[1]
                # //*[@id="ContentPlaceHolder3_GridBlock76063"]/tbody/tr[1]
                ...
                for i in range(data_len):
                    ...
                    try:
                        drug_name = data_html.xpath(
                            '//*[@id="ContentPlaceHolder3_GridBlock76063"]/tr[%s]/td[2]/text()' % (i + 2))[0].strip()
                        if not drug_name:
                            drug_name = 1
                    except:
                        drug_name = 1
                    ...
                    if drug_name != 1:
                        ...
                        try:
                            # 有效期至
                            valid_till = data_html.xpath(
                                '//*[@id="ContentPlaceHolder3_GridBlock76063"]/tr[%s]/td[8]/text()' % (i + 2))[
                                0].strip()
                        except:
                            valid_till = '2000-01-01'
                ...
                if MONTHS == 0:
                    self.cursor.execute(
                        "SELECT count(*) from order_metadata_huatongliansuoyaodian WHERE sell_time='{}' and delivery_name='{}'".format(
                            self.yesterday, enterprise_name))
                else:
                    month = int(str(self.fist).replace('-', '')[0: 6])
                    self.cursor.execute(
                        "SELECT count(*) from order_metadata_huatongliansuoyaodian WHERE month='{}' and delivery_name='{}'".format(
                ...
                sql_data_crm_record = sql_crm_record.format(company_id, delivery_id, enterprise_name,
                                                            get_account, '30-huatongliansuoyaodian',
                ...
                print('huatongliansuoyaodian-登入失败:%s' % e)
