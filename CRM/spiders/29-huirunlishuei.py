...
class HuirunlishueiSpider(scrapy.Spider):
    name = 'huirunlishuei'
    allowed_domains = ['.com']
    start_urls = ['http://www.zjlszxyy.com:8080/syslogin.aspx?result=1&username=xsx&txtCompanyID=28']
    headers = HEADERS
    time_stamp = str(int(time.time() * 1000))
    yesterday = YESTERDAY
    fist = FIST
    last = LAST
    sess_huirunlishuei = requests.session()
    ...
    def parse(self, response):
        self.number += 1
        # delivery_id = 'F617B115D6F3447983E94BB781231251'
        delivery_id = 'DDA100101L'
        ...
        if is_enable == 1:
            sell_time = 0
            html = self.sess_huirunlishuei.get(url=self.start_urls[0], headers=self.headers, verify=False)
            resp = etree.HTML(html.content.decode('utf-8'))
            __VIEWSTATE = resp.xpath('//*[@id="__VIEWSTATE"]/@value')[0]
            image = self.sess_huirunlishuei.get(
            ...
            if SCRAPYD_TYPE == 1:
                ...
                # print('path', path)
                files = r'{}{}static{}29-huirunlishuei'.format(path, symbol, symbol)
                if not os.path.exists(files):
                    os.makedirs(files)
                with open(r'{}{}static{}29-huirunlishuei{}yzm.jpg'.format(path, symbol, symbol, symbol), 'wb') as f:
                    f.write(image.content)
                # 图片文件
                filename = r'{}{}static{}29-huirunlishuei{}yzm.jpg'.format(path, symbol, symbol, symbol)
            else:
                with open(r'./29-huirunlishueiyzm.jpg', 'wb') as f:
                    f.write(image.content)
                # 图片文件
                filename = r'./29-huirunlishueiyzm.jpg'
            ...
            self.sess_huirunlishuei.post(url="http://www.zjlszxyy.com:8080/syslogin.aspx?result=5&txtCompanyID=28",
                                         data=data, headers=self.headers, verify=False)
            psot_data_html = self.sess_huirunlishuei.get(
                url='http://app1.yy5u.com:8080/ReportServer/Pages/ReportViewer.aspx?/BNumberTrafficFlowQuery&rs:Command=Render&rc:Parameters=false&SysCompanyID=28&UserID=CUSR201710311357&BizType=&GoodsID=&BeginDate={}&EndDate={}&serverNames=192.168.18.1&sqlName=NetSrv_App3&userName=mztZ8O0gn1HUBnbz9wW68Q%3d%3d&pass=HYg8AE7GMeP2W2YGWaIEpg%3d%3d&CustomerName=&BatchNo=&DepartName='.format(
                    self.fist, self.last),
                headers=self.headers,
                verify=False)
            ...
            data_data = {
                ...
                "ReportViewerControl$ctl04$ctl11$txtValue": "28",
                "ReportViewerControl$ctl04$ctl13$txtValue": "CUSR201710311357",
                "ReportViewerControl$ctl04$ctl15$txtValue": "",
                "ReportViewerControl$ctl04$ctl17$txtValue": "",
                # "ReportViewerControl$ctl04$ctl19$txtValue": "2018-07-01",
                "ReportViewerControl$ctl04$ctl19$txtValue": self.fist,
                "ReportViewerControl$ctl04$ctl21$txtValue": self.last,
                # "ReportViewerControl$ctl04$ctl21$txtValue": "2018-10-26",
            ...
            # print('-' * 1000)
            data_html = self.sess_huirunlishuei.post(
                url='http://app1.yy5u.com:8080/ReportServer/Pages/ReportViewer.aspx?%2fBNumberTrafficFlowQuery&rs%3aCommand=Render&rc%3aParameters=false&SysCompanyID=28&UserID=CUSR201710311357&BizType=&GoodsID=&BeginDate={}&EndDate={}&serverNames=192.168.18.1&sqlName=NetSrv_App3&userName=mztZ8O0gn1HUBnbz9wW68Q%3d%3d&pass=HYg8AE7GMeP2W2YGWaIEpg%3d%3d&CustomerName=&BatchNo=&DepartName='.format(
                    self.fist, self.last),
            ...
            try:
                ...
                for i in range(data_len):
                    ...
                    bill_type = 3
                    # 表的名称
                    table_name = 'order_metadata_huirunlishuei'
                    try:
                    ...
                    if drug_name != 0:
                        ...
                        sql_crm = "insert into order_metadata_huirunlishuei(company_id, delivery_id, delivery_name, data_version, data_type, bill_type, drug_name, drug_price_sum, department, drug_specification, supplier_name, drug_unit, drug_number, drug_batch, valid_till, hospital_name, hospital_address, drug_price, sell_time, create_time, update_time, business_number, customer_area, drug_hash, hospital_hash, stream_hash, month) values('{}', '{}', '{}', '{}', {}, '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', {})"
                        ...
                        self.cursor.execute('select max(id) from order_metadata_huirunlishuei')
                ...
                for i in range(2, 1000):
                    ...
                    data_htmls = self.sess_huirunlishuei.post(
                        url='http://app1.yy5u.com:8080/ReportServer/Pages/ReportViewer.aspx?%2fBNumberTrafficFlowQuery&rs%3aCommand=Render&rc%3aParameters=false&SysCompanyID=28&UserID=CUSR201710311357&BizType=&GoodsID=&BeginDate={}&EndDate={}&serverNames=192.168.18.1&sqlName=NetSrv_App3&userName=mztZ8O0gn1HUBnbz9wW68Q%3d%3d&pass=HYg8AE7GMeP2W2YGWaIEpg%3d%3d&CustomerName=&BatchNo=&DepartName='.format(
                            self.fist, self.last),
                        # url='http://app1.yy5u.com:8080/ReportServer/Pages/ReportViewer.aspx?%2fBNumberTrafficFlowQuery&rs%3aCommand=Render&rc%3aParameters=false&SysCompanyID=28&UserID=CUSR201710311357&BizType=&GoodsID=&BeginDate=2018-07-01&EndDate=2018-10-26&serverNames=192.168.18.1&sqlName=NetSrv_App3&userName=mztZ8O0gn1HUBnbz9wW68Q%3d%3d&pass=HYg8AE7GMeP2W2YGWaIEpg%3d%3d&CustomerName=&BatchNo=&DepartName=',
                    ...
                    if sell_time == 0:
                        ...
                        if MONTHS == 0:
                            self.cursor.execute(
                                "SELECT count(*) from order_metadata_huirunlishuei WHERE sell_time='{}' and delivery_name='{}'".format(
                                    self.yesterday, enterprise_name))
                        else:
                            month = int(str(self.fist).replace('-', '')[0: 6])
                            self.cursor.execute(
                                "SELECT count(*) from order_metadata_huirunlishuei WHERE month='{}' and delivery_name='{}'".format(
