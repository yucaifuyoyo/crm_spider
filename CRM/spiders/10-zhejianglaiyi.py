...
class ZhejianglaiyiSpider(scrapy.Spider):
    ...
    fist = FIST
    last = LAST
    sess_zhejianglaiyi = requests.session()
    ...
    def parse(self, response):
        ...
        if is_enable == 1:
            self.number += 1
            html = self.sess_zhejianglaiyi.get(url=self.start_urls[0], headers=self.headers, verify=False)
            image = self.sess_zhejianglaiyi.get(url='http://www.600216.com/lx/bmp.asp?flg=login', headers=self.headers,
                                                verify=False)
            ...
            if SCRAPYD_TYPE == 1:
                ...
                files = r'{}{}static{}10-zhejianglaiyi'.format(path, symbol, symbol)
                if not os.path.exists(files):
                    os.makedirs(files)
                # print('path', path)
                with open(r'{}{}static{}10-zhejianglaiyi{}yzm.jpg'.format(path, symbol, symbol, symbol), 'wb') as f:
                    f.write(image.content)
                # 图片文件
                filename = r'{}{}static{}10-zhejianglaiyi{}yzm.jpg'.format(path, symbol, symbol, symbol)
            else:
            ...
            self.sess_zhejianglaiyi.post(url='http://www.600216.com/lx/index.asp', data=data, headers=self.headers,
                                         verify=False)
            data_html = self.sess_zhejianglaiyi.get(url='http://www.600216.com/lx/sa.asp?fir=1', headers=self.headers,
                                                    verify=False)
            try:
