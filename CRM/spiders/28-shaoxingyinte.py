...
class ShaoxinyinteSpider(scrapy.Spider):
    name = 'shaoxingyinte'
    allowed_domains = ['http://www.drugoogle.com/index/index.htm']
    ...
    fist = FIST
    last = LAST
    sess_shaoxingyinte = requests.session()
    ...
    def parse(self, response):
        # delivery_id = 'F617B115D6F3447983E94BB781231246'
        delivery_id = 'DDA100101J'
        ...
        if is_enable == 1:
            self.number += 1
            html = self.sess_shaoxingyinte.get(url=self.start_urls[0], headers=self.headers, verify=False)
            image = self.sess_shaoxingyinte.get(
            ...
            if SCRAPYD_TYPE == 1:
                ...
                # print('path', path)
                files = r'{}{}static{}28-shaoxingyinte'.format(path, symbol, symbol)
                if not os.path.exists(files):
                    os.makedirs(files)
                with open(r'{}{}static{}28-shaoxingyinte{}yzm.jpg'.format(path, symbol, symbol, symbol), 'wb') as f:
                    f.write(image.content)
                # 图片文件
                filename = r'{}{}static{}28-shaoxingyinte{}yzm.jpg'.format(path, symbol, symbol, symbol)
            else:
                with open(r'./28-shaoxingyinteyzm.jpg', 'wb') as f:
                    f.write(image.content)
                # 图片文件
                filename = r'./28-shaoxingyinteyzm.jpg'
            ...
            data = {"username": get_account,
                    ...
                    "verifyCode": yzm}
            # print('data', data)
            self.sess_shaoxingyinte.post(
                url="http://www.drugoogle.com/index/registerloginjson.jspx?%s" % (int(time.time() * 1000)),
                data=data, headers=self.headers, verify=False)
            data_html = self.sess_shaoxingyinte.get(url='http://www.drugoogle.com/member/index.jspx?catlog=4',
                                                    headers=self.headers,
                                                    verify=False)
            try:
                # print("data_html.content.decode('utf-8')", data_html.content.decode('utf-8'))
                re.findall(r'药品流向查询', data_html.content.decode('utf-8'))[0]
                data_html = self.sess_shaoxingyinte.get(
                    url='http://www.drugoogle.com/member/agentman/medicineGoto/medicinegototab4.jspx?entryId=21&medicineId=0&company_name=&timeType=1&startTime={}%2000:00:00&endTime={}%2023:59:59&buyerType=0'.format(
                        self.fist, self.last),
                    # url='http://www.drugoogle.com/member/agentman/medicineGoto/medicinegototab4.jspx?entryId=21&medicineId=0&company_name=&timeType=1&startTime=2018-10-01%2000:00:00&endTime=2018-10-25%2023:59:59&buyerType=0',
                ...
                for i in range(data_len):
                    ...
                    if drug_name != 1:
                        ...
                        table_name = 'order_metadata_yingtehaishi'
                        ...
                        sql_crm = "insert into order_metadata_shaoxingyinte(company_id, delivery_id, delivery_name, data_version, data_type, bill_type, drug_name, trade_id, sales_ticket_id, drug_specification, supplier_name, drug_unit, drug_number, drug_batch, valid_till, hospital_name, hospital_address, sell_time, warehouse_time, create_time, update_time, drug_hash, hospital_hash, stream_hash, drug_price, month) values('{}', '{}', '{}', '{}', {}, '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', {})"
                        ...
                        self.cursor.execute('select max(id) from order_metadata_shaoxingyinte')
                ...
                data_html_cai = self.sess_shaoxingyinte.get(
                    url='http://www.drugoogle.com/member/agentman/medicineGoto/medicinegototab3.jspx?entryId=21&medicineId=0&timeType=1&startTime={}%2000:00:00&endTime={}%2023:59:59'.format(
                        self.fist, self.last),
                    # url='http://www.drugoogle.com/member/agentman/medicineGoto/medicinegototab3.jspx?entryId=21&medicineId=0&timeType=1&startTime=2018-10-01%2000:00:00&endTime=2018-10-25%2023:59:59',
                    headers=self.headers, verify=False)
                ...
                if MONTHS == 0:
                    self.cursor.execute(
                        "SELECT count(*) from order_metadata_shaoxingyinte WHERE sell_time='{}' and delivery_name='{}'".format(
                            self.yesterday, enterprise_name))
                else:
                    month = int(str(self.fist).replace('-', '')[0: 6])
                    self.cursor.execute(
                        "SELECT count(*) from order_metadata_shaoxingyinte WHERE month='{}' and delivery_name='{}'".format(
