...
class WenzhouyingteSpider(scrapy.Spider):
    ...
    # last = '2018-11-05'
    last = LAST
    sess_wenzhouyingte = requests.session()
    ...
    def parse(self, response):
        # delivery_id = 'F617B115D6F3447983E94BB781231232'
        delivery_id = 'DDA1001007'
        ...
        if is_enable == 1:
            self.number += 1
            login_html = self.sess_wenzhouyingte.get(url=self.start_urls[0], headers=self.headers, verify=False)
            ...
            login_html = self.sess_wenzhouyingte.post(url='http://www.wzmed.cn/Login.aspx', data=login_data,
                                                      headers=self.headers,
                                                      verify=False)
            # print('login_html', login_html.content.decode('utf-8'))
            psot_data_html = self.sess_wenzhouyingte.get(url='http://www.wzmed.cn/Sales.aspx', headers=self.headers,
                                                         verify=False)
            ...
            try:
                ...
                self.sess_wenzhouyingte.post(url='http://www.wzmed.cn/Sales.aspx', data=data_data, headers=self.headers,
                                             verify=False)
                data_html = self.sess_wenzhouyingte.get(url='http://www.wzmed.cn/SaleList.aspx', headers=self.headers,
                                                        verify=False)
                ...
                # -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                psot_data_html_cai = self.sess_wenzhouyingte.get(url='http://www.wzmed.cn/Stock.aspx',
                ...
                data_data = dict(data_data, **self.gvGoods)
                self.sess_wenzhouyingte.post(url='http://www.wzmed.cn/Stock.aspx', data=data_data, headers=self.headers,
                                             verify=False)
                data_html_cai = self.sess_wenzhouyingte.get(url='http://www.wzmed.cn/PurchaseList.aspx',
                                                            headers=self.headers,
                                                            verify=False)
